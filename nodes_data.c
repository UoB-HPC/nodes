...
void build_unstructured_quad_mesh(UnstructuredMesh* mesh, const int global_nx,
...
void initialise_rectilinear_quad_mesh_2d(UnstructuredMesh* unstructured_mesh,
  ...
  allocate_int_data(&unstructured_mesh->edge_vertex0,
                    unstructured_mesh->nedges);
  allocate_int_data(&unstructured_mesh->edge_vertex1,
                    unstructured_mesh->nedges);
  allocate_int_data(&unstructured_mesh->edges_cells,
                    unstructured_mesh->nedges * NCELLS_PER_EDGE);
  ...
  for (int ii = 0; ii < ny + 1; ++ii) {
    ...
    if (ii < ny) {
      for (int jj = 0; jj < nx + 1; ++jj) {
        ...
        unstructured_mesh->edge_vertex0[edge_index] = (ii) * (nx + 1) + (jj);
        unstructured_mesh->edge_vertex1[edge_index] =
            (ii + 1) * (nx + 1) + (jj);
      }
    }
  }
  ...
  for (int ii = 0; ii < ny + 1; ++ii) {
    for (int jj = 0; jj < nx; ++jj) {
      const int edge_index = (ii) * (2 * nx + 1) + (jj);
      unstructured_mesh->edges_cells[(edge_index)*NCELLS_PER_EDGE + 0] =
          (ii)*nx + (jj);
      unstructured_mesh->edges_cells[(edge_index)*NCELLS_PER_EDGE + 1] =
          (ii - 1) * nx + (jj);
    }
    if (ii < ny) {
      for (int jj = 0; jj < nx + 1; ++jj) {
  ...
  for (int ii = 0; ii < ny; ++ii) {
    for (int jj = 0; jj < nx; ++jj) {
      unstructured_mesh->cells_edges[(BOTTOM)*nx * ny + (ii)*nx + (jj)] =
          (ii) * (2 * nx + 1) + (jj);
  ...
  for (int ii = 0; ii < ny; ++ii) {
    for (int jj = 0; jj < nx; ++jj) {
      ...
      for (int kk = 0; kk < NEDGES; ++kk) {
        int edge_index =
            unstructured_mesh->cells_edges[(kk)*nx * ny + (cell_index)];
      ...
      unstructured_mesh->cell_centroids_x[cell_index] =
          (1.0 / (6.0 * A)) * c_x_factor;
      unstructured_mesh->cell_centroids_y[cell_index] =
